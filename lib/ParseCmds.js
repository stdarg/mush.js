...
ParseCmds.prototype.parseCommand = function(cmdEntry) {
    ...
    var cmd = undefined;
    var shortCmd = false;
    log.info('cmdEntry: '+util.inspect(cmdEntry));
...
function _cmdIsValid(cmd, cmdEntry) {
    ...
    if (cmdEntry.conn.player.isDisconnected() && cmd.dconPlayer === false) {
        //log.info('cmdEntry.conn.player: '+util.inspect(cmdEntry.conn.player));
        //log.info('cmd.dconPlayer: '+util.inspect(cmd.dconPlayer));
        return false;
    }
