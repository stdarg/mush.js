...
/**
 * Creates a new empty BitArray, or initialises the BitArray with the given BitArray
 * serialisation (an Array of integers.)
 */
...
BitArray.prototype.set = function (index, value) {
	...
	// Since "undefined | 1 << index" is equivalent to "0 | 1 << index" we do not need to
    // initialise the array explicitly here.
	if (value) {
...
/**
 * Toggles the bit at index. If the bit is on, it is turned off. Likewise, if the bit is
 * off it is turned on.
 */
...
BitArray.prototype.count = function () {
	...
	// If we remove the toggle method we could efficiently cache the number of bits
