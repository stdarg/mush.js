/**
 * @fileOverview The CommandTable is an object for storing and looking commands and aliases to
 * commands. The commands and aliases exist as propeties on an object, so the lookup time is
 * O(1). The entry for each command defines how the mush uses the command, e.g. how many
 * arguments, the function handler and wheather or not connected/disconnected player can use the
 * command.
 */
'use strict';
...
// The below table may seem awkward for an object. However, in node, the JS 'global scope' is confined to a module 
// (file). Without the exports, it is not visible outside the module. The means the data is like a static in C, 
// so many instances of CommandTable can exist, but all will refer to the same data. Though, in our case, I expect 
// only 1 instance.
var cmdTab = {};
...
function CommandTable() {
    ...
    this._addCmd('connect', 2, Commands.prototype.connect, noConPlayer,  yesDconPlayer);
    this._makeAlias('connect', 'connec');
    this._makeAlias('connect', 'conne');
    this._makeAlias('connect', 'conn');
    this._makeAlias('connect', 'con');
    this._makeAlias('connect', 'co');
    this._makeAlias('connect', 'c');

    this._addCmd('create',  2, Commands.prototype.create,  noConPlayer,  yesDconPlayer);
    this._makeAlias('create', 'creat');
    this._makeAlias('create', 'crea');
    this._makeAlias('create', 'cre');
    this._makeAlias('create', 'cr');

    this._addCmd('look',  -1, Commands.prototype.look,     yesConPlayer,  noDconPlayer);
    this._makeAlias('look', 'loo');
    this._makeAlias('look', 'lo');
    this._makeAlias('look', 'l');

    this._addCmd('pose',    1, Commands.prototype.pose,    yesConPlayer, noDconPlayer );
    this._makeAlias('pose', 'pos');
    this._makeAlias('pose', 'po');
    this._makeAlias('pose', 'p');

    // we don't want to alias quit to shortened versions, to prevent accidental quit-tation.
    this._addCmd('quit',    0, Commands.prototype.quit,    yesConPlayer, yesDconPlayer);
    this._makeAlias('quit', 'QUIT');
    this._makeAlias('quit', 'LOGOUT');
    this._makeAlias('quit', 'logout');

    this._addCmd('say',     1, Commands.prototype.say,     yesConPlayer, noDconPlayer );
    this._makeAlias('say', 'sa');
    this._makeAlias('say', 's');

    this._addCmd('who',     0, Commands.prototype.who,     yesConPlayer, yesDconPlayer);
    this._makeAlias('who', 'WHO');
    this._makeAlias('who', 'wh');
    this._makeAlias('who', 'w');
