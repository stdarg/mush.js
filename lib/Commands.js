...
Commands.prototype.describe = function(cmdEntry) {
    ...
    // NOTE: Tricky. @describe is different. It allows the following syntax:
    // @describe me=<text>

    log.info('cmdEntry: %s', util.inspect(cmdEntry));
    ...
    var target = ary[0];
    var descText = ary[1];
    log.error('target="%s" desc="%s"', target, descText);
